...
"#;
        ...
        assert_eq!(
            value.get("default_agent").and_then(|v| v.as_str()),
            Some("nexus")
        );
    }
...
"#;
    ...
    fn test_provider_types() {
        let providers = vec!["anthropic", "openai", "openai_compatible", "ollama"];
    ...
    fn test_max_tokens_bounds() {
        ...
        for tokens in &valid_tokens {
            ...
            assert!(
                *tokens <= 128000,
                "max_tokens should not exceed context limit"
            );
        }
    }
    ...
    fn test_env_var_patterns() {
        let env_vars = vec!["ANTHROPIC_API_KEY", "OPENAI_API_KEY", "OLLAMA_HOST"];
