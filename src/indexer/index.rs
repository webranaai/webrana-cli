...
impl FileIndex {
    ...
    pub fn build(entries: Vec<FileEntry>) -> Self {
        ...
        index.code_files = entries.iter().filter(|e| e.file_type == FileType::Code).count();
        index.config_files = entries.iter().filter(|e| e.file_type == FileType::Config).count();
    ...
    pub fn summary(&self) -> String {
        let dirs = self.entries.iter().filter(|e| e.file_type == FileType::Directory).count();
